<!-- Modal -->
<div class="modal fade" id="coordinatorModal" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form method="post" class="bg-light border border-primary rounded p-5">
        ...
        <!-- ADD STUDENT MODAL -->
        <h1 class="text-center fw-bold mb-5">Add COORDINATOR</h1>
        ...
        <div class="d-flex justify-content-around mt-4">
          <input type="submit" class="btn btn-success btn-lg" name="addingCoordinatorSubmit" value="Add"></input>
...
if (isset($_POST['addingCoordinatorSubmit'])) {

  $coordinatorschool_id = $_POST['schoolID'];
  ...
  $checkSchoolID = mysqli_query($con, "SELECT *  FROM cms_users where school_id = '$coordinatorschool_id'");
...
<?php
    ...
    $res = $con->query("INSERT INTO `cms_coordinators`(`school_id`, `first_name`, `last_name`, `status`) 
  VALUES ('$coordinatorschool_id','$fname','$lname', 1)");

   $sql = "INSERT INTO `cms_users`(`school_id`, `first_name`, `last_name`, `password`, `role`) 
    VALUES ('$coordinatorschool_id', '$fname', '$lname', '$hashedpassword', 'coordinator')";
    $con->query($sql);

    $sql = "SELECT * FROM cms_users WHERE school_id='$coordinatorschool_id' AND first_name='$fname'";
    $res = $con->query($sql);

    if(mysqli_num_rows($res)>0){
      while($row = mysqli_fetch_assoc($res)){
        $coordinatorschool_id = $row['school_id'];
        $sql2 = "INSERT INTO `cms_profileimg`(`school_id`, `status`) VALUES ('$coordinatorschool_id','1')";
        $con->query($sql2);
      }
